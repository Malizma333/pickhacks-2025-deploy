import { IMember } from '@/interfaces/IMember';
import { picklogo25, rahulg, maggief, tyler, daniellep, joshuak, mattm, marcellat, chase, jadac, rahulv, nathand, lindseyc, ericd, everetty, jasonx, ericw, vishaanthm, shrijam, laurenh } from './Images';

export const Members: IMember[] = [
	// Director
	{
		name: 'Vishaanth Muddu',
		linkedin: 'https://www.linkedin.com/in/vishaanth-muddu',
		role: 'Director',
		avatar: vishaanthm.src
	},
	// Team Leads
	{
		name: 'Eric Wong',
		linkedin: 'https://www.linkedin.com/in/ewbyf/',
		role: 'Development Lead',
		avatar: ericw.src
	},
	{
		name: 'Shrija Maganti',
		linkedin: 'https://www.linkedin.com/in/shrija-maganti/',
		role: 'Logistics Lead',
		avatar: shrijam.src
	},
    {
		name: 'Lauren Higgins',
		linkedin: 'https://www.linkedin.com/in/higgins7loh/',
		role: 'Design Lead',
		avatar: laurenh.src
	},
    {
		name: 'Rahul Gopalan',
		linkedin: 'https://www.linkedin.com/in/rahulgopalan/',
		role: 'Finance Lead',
		avatar: rahulg.src
	},
	// Dev Members
	{
		name: 'Jason Xu',
		linkedin: 'https://www.linkedin.com/in/jason-xu-317585220/',
		role: 'Development Member',
		avatar: jasonx.src
	},
	{
		name: 'Everett Yan',
		linkedin: 'https://www.linkedin.com/in/ezybg7/',
		role: 'Development Member',
		avatar: everetty.src
	},
	// {
	// 	name: 'Reid Haegele',
	// 	linkedin: 'https://www.linkedin.com/in/reid-haegele/',
	// 	role: 'Development Member',
	// 	avatar: picklogo25.src
	// },
	{
		name: 'Eric Duong',
		linkedin: 'https://www.linkedin.com/in/eric-duong-939604241',
		role: 'Development Member',
		avatar: ericd.src
	},
	// {
	// 	name: 'Tung Nguyen',
	// 	linkedin: 'https://www.linkedin.com/in/tung-nguyen-0a509b26a/',
	// 	role: 'Development Member',
	// 	avatar: picklogo25.src
	// },
	// Log Members
	{
		name: 'Matt Minatra',
		linkedin: 'https://www.linkedin.com/in/mjminatra/',
		role: 'Logistics Member',
		avatar: mattm.src
	},
	{
		name: 'Lindsey Chan',
		linkedin: 'https://www.linkedin.com/in/lindsey-chan/',
		role: 'Logistics Member',
		avatar: lindseyc.src
	},
	{
		name: 'Nathan Derucki',
		linkedin: 'https://www.linkedin.com/in/nathan-derucki/',
		role: 'Logistics Member',
		avatar: nathand.src
	},
	{
		name: 'Rahul Vinay',
		linkedin: 'https://www.linkedin.com/in/rahul-vinay/',
		role: 'Logistics Member',
		avatar: rahulv.src
	},
	{
		name: 'Jada Castille',
		linkedin: 'https://www.linkedin.com/in/jada-castile-1208a6291',
		role: 'Logistics Member',
		avatar: jadac.src
	},
	{
		name: 'Chase Jankowski',
		linkedin: 'https://www.linkedin.com/in/chase-jankowski-52553428a/',
		role: 'Logistics Member',
		avatar: chase.src
	},
	// Design Members
	{
		name: 'Marcella Tebeau',
		linkedin: 'https://www.linkedin.com/in/marcella-tebeau/',
		role: 'Design Member',
		avatar: marcellat.src
	},
	// {
	// 	name: 'Keith Miller',
	// 	linkedin: 'https://www.linkedin.com/in/keith-miller-22387424b/',
	// 	role: 'Design Member',
	// 	avatar: picklogo25.src
	// },
	{
		name: 'Joshua Kroft',
		linkedin: 'https://www.linkedin.com/in/joshua-kroft-b54909257/',
		role: 'Design Member',
		avatar: joshuak.src
	},
	// Finance Members
	{
		name: 'Maggie Fu',
		linkedin: 'https://linkedin.com/in/maggie-fu1809',
		role: 'Finance Member',
		avatar: maggief.src
	},
	{
		name: 'Tyler Sanford',
		linkedin: 'http://www.linkedin.com/in/tyler-sanford-1249402b7',
		role: 'Finance Member',
		avatar: tyler.src
	},
	{
		name: 'Danielle Perkins',
		linkedin: 'https://www.linkedin.com/in/danielle-perkins-cyber/',
		role: 'Finance Member',
		avatar: daniellep.src
	},
	// Marketing Members
];
